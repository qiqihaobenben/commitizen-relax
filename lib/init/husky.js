...
function updateHooks(rootDir, isForce) {
    ...
    if (fs_extra_1.default.existsSync(prepareCommitMsgPath)) {
        if (isForce) {
            console.log(chalk_1.default.yellow("using --force\uFF0Cthe .husky/prepare-commit-msg will override."));
            fs_extra_1.default.removeSync(prepareCommitMsgPath);
        }
        else {
            console.log(chalk_1.default.yellow("The file already exists: .husky/prepare-commit-msg, husky is append, not override.If you want to override hooks file, please use Use --force to override."));
        }
    }
    if (fs_extra_1.default.existsSync(commitMsgPath)) {
        if (isForce) {
            console.log(chalk_1.default.yellow("using --force\uFF0Cthe .husky/commit-msg will override."));
            fs_extra_1.default.removeSync(commitMsgPath);
        }
        else {
            console.log(chalk_1.default.yellow("he file already exists: .husky/commit-msg, husky is append , not override.If you want to override hooks file, please use Use --force to override."));
        }
    }
    // TODO 添加一些导语
